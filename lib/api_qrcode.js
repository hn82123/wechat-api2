...
exports._createTmpQRCode = function (sceneId, expire, callback) {
  ...
  console.log('isWXCloud:', this.isWXCloud);

  var url = this.endpoint + '/cgi-bin/qrcode/create'
  if(this.isWXCloud)
    url += '?from_appid=' + this.appid;
  else
    url += '?access_token=' + this.token.accessToken;
  ...
  this.request(url, postJSON(data), function(err, result, res) {
    console.log('响应结果:', result);
    console.log('响应错误:', err);
    wrapper(callback)(err, result, res);
  });
};
